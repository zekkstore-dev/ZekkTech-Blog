...
export default function ShareBox({ title, slug }: ShareBoxProps) {
  ...
  // pake useEffect biar URL cuma dibikin di client (hindari hydration mismatch)
  useEffect(() => {
  ...
  // === format link share buat masing-masing platform ===
  ...
  // twitter / x
  const tweetText = `${title} — baca selengkapnya di ZekkTech Blog 🚀`;
  const twitterLink = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}&url=${encodeURIComponent(articleUrl)}`;
  ...
  // fungsi salin link ke clipboard user
  const handleCopy = async () => {
    try {
      ...
      // fallback kalo clipboard API ga support
  ...
  // data tombol sosmed biar gampang di-loop
  const socials = [
    { name: 'WhatsApp', href: waLink, color: 'bg-[#25D366] hover:bg-[#1ebe57]', icon: (
    ...
    { name: 'Twitter', href: twitterLink, color: 'bg-black hover:bg-gray-800', icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    )},
    { name: 'Facebook', href: fbLink, color: 'bg-[#1877F2] hover:bg-[#166fe5]', icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    )},
    { name: 'Telegram', href: tgLink, color: 'bg-[#0088cc] hover:bg-[#0077b5]', icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0a12 12 0 00-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
      </svg>
    )},
    { name: 'LinkedIn', href: linkedinLink, color: 'bg-[#0A66C2] hover:bg-[#004182]', icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="flex items-center gap-2 mb-4">
        {socials.map(s => (
          <a
            ...
            className={`inline-flex items-center justify-center w-9 h-9 rounded-full ${s.color} text-white transition-all hover:scale-110 hover:shadow-lg`}
            title={`Bagikan via ${s.name}`}
          >
            {s.icon}
          </a>
        ))}
