...
export default function SubscribeSection() {
  ...
  return (
    <section id="subscribe" className="w-full bg-[var(--bg-secondary)] border-t border-[var(--border-primary)] py-16 sm:py-20 scroll-mt-20 transition-colors duration-300">
      <div className="max-w-[680px] mx-auto px-6 text-center">

        {/* Icon */}
        <div className="w-12 h-12 bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/30 flex items-center justify-center mx-auto mb-5 rounded-lg">
        ...
        <p className="text-[14px] sm:text-[15px] text-[var(--text-secondary)] mb-2 leading-relaxed">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8 text-[12px] text-[var(--text-secondary)]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full max-w-[500px] mx-auto">
          <input
            ...
            className="w-full h-[52px] px-5 bg-[var(--bg-input)] border border-[var(--border-secondary)] rounded-xl text-[14px] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 transition-all disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={loading || !email.trim() || !token || status === 'success'}
            className="w-full h-[52px] px-8 bg-blue-500 hover:bg-blue-600 text-white text-[15px] font-semibold rounded-xl transition-all hover:shadow-lg hover:shadow-blue-500/20 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed"
          >
